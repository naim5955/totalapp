...
# PAGE B: COST ESTIMATION CALCULATOR (DAMPPROOF WALL)
    ...
    # Option 1: Additional cost is the difference between Traditional Lifetime and Holcim Premium Initial
    additional_lifetime_cost = lifetime_std_total - lifetime_hwp_total
    total_net_savings = additional_lifetime_cost
    ...
    st.markdown(f"""
        ...
        <div class="tco-card" style="border-left: 6px solid #EF4444;">
            ...
            <div class="card-value" style="color: #DC2626;">BDT {additional_lifetime_cost:,.0f}</div>
            <div class="card-sub">Net extra financial loss over Holcim Premium across {failure_years} years</div>
        </div>
        
        <div class="tco-card" style="border-left: 6px solid #10B981; background: linear-gradient(135deg, #FFFFFF 0%, #F0FDF4 100%);">
            <div class="card-title" style="color: #065F46;">TOTAL NET SAVINGS (ROI)</div>
            <div class="card-value" style="color: #047857;">BDT {total_net_savings:,.0f} ({annual_roi_percentage:,.1f}% / Year)</div>
            <div class="card-sub">Cumulative savings over {failure_years} years against total Holcim Premium plan investment of BDT {initial_hwp_total:,.0f} (Total ROI: {total_roi_percentage:,.1f}%)</div>
        </div>
    """, unsafe_allow_html=True)
